...
| **Severity** | Medium |
| **Status** | **FIXED** — `LocalOverrideManager` now stores FCC credential overrides as Keystore-encrypted blobs using `KeystoreManager.storeSecret(KeystoreManager.ALIAS_FCC_CRED, ...)`, persists only Base64 ciphertext in preferences, deletes the alias on clear/reset, and migrates legacy plaintext overrides on read. |
...
| **Severity** | Low |
| **Status** | **FIXED** — Removed the pre-parse QR scan info log, downgraded QR parse failure logging to a message-only warning (no exception payload/stack trace), and annotated `QrBootstrapData.provisioningToken` with `@Sensitive` so the existing redaction pipeline treats it as secret. |
...
| **Module** | Local API |
| **Severity** | High |
| **Status** | **FIXED** — `LocalApiServer` now fails closed on Android: when `enableLanApi=true`, the server does **not** expose `0.0.0.0` over cleartext HTTP and remains bound to `127.0.0.1` until secure transport is actually implemented. This removes the cleartext LAN exposure rather than merely warning about it. |
...
| **Severity** | Medium |
| **Status** | **FIXED** — Populated `network_security_config.xml` with the real bootstrap SHA-256 pins and aligned `HttpCloudApiClient.BUNDLED_PINS` / `AppModule` to the same source of truth, so Android framework pinning now matches the OkHttp bootstrap pin set. |
...
| **Module** | FCC Adapters |
| **Severity** | Medium |
| **Status** | **FIXED** — Advatec and Petronite now resolve FCC HTTP endpoints with secure-by-default rules: bare non-loopback hosts default to HTTPS, explicit non-loopback `http://` endpoints are rejected, and only loopback/localhost cleartext remains allowed for same-device deployments and simulators. |
...
| **Severity** | Medium |
| **Status** | **FIXED** — `RadixSignatureHelper` now emits a one-time runtime security warning whenever the protocol-mandated SHA-1 signer/verifier is used, making the vendor limitation explicit in operational logs while preserving interoperability with current Radix devices. |
...
| **Severity** | Medium |
| **Status** | **FIXED** — `AdvatecWebhookListener` now rejects empty bodies with HTTP 400, missing/invalid webhook tokens with HTTP 401, preserves HTTP 429 for queue back-pressure, and returns HTTP 500 on unexpected handler failures instead of masking all cases behind `200 OK`. |
| **Category** | Missing API Auth Headers |
...
| **Severity** | Medium |
| **Status** | **FIXED** — `PreAuthHandler` now Keystore-encrypts `customerTaxId` before persistence and stores only an `ENCv1:` Base64 ciphertext in Room. `PreAuthCloudForwardWorker` decrypts the value only when building the cloud payload and re-encrypts legacy plaintext rows before forwarding them. |
...
| **Module** | Security |
| **Severity** | Low |
| **Status** | **FIXED** — Added an explicit `network_security_config.xml` comment documenting that Android framework cleartext policy does not govern the direct FCC adapter clients, and that loopback/localhost cleartext is the only intentional exception while non-loopback FCC endpoints are enforced as HTTPS in `FccTransportSecurity`. |
...
| **Module** | Provisioning & Lifecycle |
| **Severity** | Low |
| **Status** | **FIXED** — Registration completion logging now omits `siteCode` and truncates `deviceId` to an 8-character prefix in `RegistrationHandler`, and `EncryptedPrefsManager.saveRegistration()` no longer logs the full site code when persisting registration state. |
| **Category** | Sensitive Data Logged |
...
| **Module** | Provisioning & Lifecycle |
| **Severity** | High |
| **Status** | **FIXED** — `HttpCloudApiClient.registerDevice()` now creates a dedicated pinned HTTP client for the QR-provided cloud hostname during bootstrap registration, so the provisioning token is sent over a hostname-bound pinned connection even before the long-lived client is rebuilt after registration. |
| **Category** | Missing API Auth Headers |
...
| **Module** | Provisioning & Lifecycle |
| **Severity** | Low |
| **Status** | **FIXED** — `RegistrationHandler` now retries config encryption once and fails closed on persistent Keystore failure: it no longer writes raw `SiteConfig` JSON into Room, and instead skips the initial cache write so the service can fetch config on the first successful poll. |
| **Category** | Insecure Token Storage |
...
| **Module** | FCC Adapters (Advatec) |
| **Severity** | Medium |
| **Status** | **FIXED** — Advatec transport is now resolved through `FccTransportSecurity`: bare non-loopback device addresses default to HTTPS, explicit non-loopback `http://` URLs are rejected, and only loopback/localhost cleartext remains permitted for same-device deployments and simulators. |
| **Category** | Sensitive Data Handling |
...
| **Module** | FCC Adapters (Radix) |
| **Severity** | Medium |
| **Status** | **FIXED** — `RadixAdapter.sendPreAuth()` and `cancelPreAuth()` now validate `FDCSIGNATURE` with `RadixXmlParser.validateAuthResponseSignature(...)` before parsing `ACKCODE`. Invalid signatures fail closed: pre-auth returns `PreAuthResultStatus.ERROR` with `"Auth response signature mismatch"` and cancel returns `false`. |
| **Category** | Missing API Auth Headers |
...
| **Severity** | Low |
| **Status** | **FIXED** — `JplMessage` now overrides `toString()` and redacts map entries whose keys contain sensitive markers such as `AccessCode`, `Secret`, or `Password`, so `FcAccessCode` no longer leaks through ordinary object logging. |
| **Category** | Sensitive Data Logged |
...
| **Severity** | Low |
| **Status** | **FIXED** — `OdooWebSocketServer` now authenticates WebSocket clients with `MessageDigest.isEqual(...)` via a dedicated helper, replacing the timing-variable Kotlin string inequality check. |
| **Category** | Insecure Token Storage |
...
| **Severity** | Low |
| **Status** | **FIXED** — `TransactionBufferManager` now encrypts `rawPayloadJson` with a Keystore-backed `KeystoreBackedStringCipher` under `KeystoreManager.ALIAS_BUFFER_RAW_PAYLOAD` before persisting it, decrypts it on read for the small number of consumers that need plaintext, and `FccEdgeApplication` runs a background migration to re-encrypt legacy plaintext rows already stored in Room. |
...
| **Severity** | Medium |
| **Category** | Missing API Auth Headers |
| **Status** | **FIXED** — `HttpCloudApiClient.BUNDLED_PINS` is populated with the bootstrap SHA-256 hashes and `AppModule` now uses `HttpCloudApiClient.BUNDLED_PINS` as the single source of truth when runtime pins have not been delivered yet. |
...
| **Severity** | Low |
| **Category** | Sensitive Data Logged |
| **Status** | **FIXED** — `TelemetryReporter.collectFccHealth()` now redacts FCC connection coordinates before serialization, reporting `fccHost="***"` and a non-sensitive sentinel port instead of the site's actual LAN IP/port while still preserving vendor and heartbeat health signals. |
...
| **Severity** | Medium |
| **Category** | Sensitive Data Logged |
| **Status** | **FIXED** — `StructuredFileLogger` now sanitizes message/extra fields before persistence, `exportRedactedLogs(...)` re-sanitizes older files while building the shared ZIP, and `DiagnosticsActivity` shows an explicit warning dialog before launching the share intent. |
...
| **Severity** | Low |
| **Category** | Sensitive Data Logged |
| **Status** | **FIXED** — Persistent JSONL log entries now store only a sanitized exception summary (`exception` simple name plus first-line stack trace message), and the log-share export path re-sanitizes legacy files so call-chain details are not exposed off-device. |
...
| **Severity** | Medium |
| **Status** | **FIXED** — `WebSocketDto` now exposes `maxFrameSizeKb` with a secure default of `64`, and `OdooWebSocketServer` applies that limit via `maxFrameSize = maxFrameSizeKb * 1024` instead of `Long.MAX_VALUE`. |
| **Category** | Insecure Network Configuration |
...
| **Module** | POS Integration (Odoo) |
| **Severity** | High |
| **Status** | **FIXED** — `WebSocketDto` now defaults to `bindAddress = "127.0.0.1"` and `requireApiKeyForLan = true`; `OdooWebSocketServer` now enforces `requireApiKeyForLan`, logs a startup warning when `sharedSecret` is blank, and blocks sensitive mutating modes (`manager_update`, `attendant_update`, `manager_manual_update`, `fp_unblock`) unless the session is authenticated with `X-Api-Key`. |
| **Category** | Missing API Auth Headers |
...
| **Module** | POS Integration (Odoo) |
| **Severity** | Low |
| **Status** | **FIXED** — `OdooWsMessageHandler.handleFpUnblock()` now returns a generic client-facing error (`"Failed to unblock pump <id>"`) while continuing to log the full exception server-side. |
| **Category** | Sensitive Data Logged |
...
| **Module** | Connectivity |
| **Severity** | Medium |
| **Status** | **FIXED** — `BoundSocketFactory` now emits a WARN when no bound network is available and a socket falls back to default OS routing. The internet probe already used the pinned `CloudApiClient`; it now also fails closed when `cloudNetwork` is null instead of probing over uncontrolled default routing. |
| **Category** | Insecure Token Storage |
...
| **Module** | Connectivity |
| **Severity** | Medium |
| **Status** | **FIXED** — `ConnectivityManager` now probes internet reachability through `CloudApiClient.healthCheck()` instead of a separate unpinned probe client, and the probe returns `false` when no bound cloud network is available. |
| **Category** | Missing API Auth Headers |
...
| **Module** | Security |
| **Severity** | High |
| **Status** | **FIXED** — `StructuredFileLogger` now sanitizes persisted log content, `AppLogger.redacted(...)` exposes `SensitiveFieldFilter` to production code, and the sensitive request/response/config DTOs override `toString()` to emit redacted output by default. |
| **Category** | Sensitive Data Logged |
...
| **Module** | Security |
| **Severity** | Medium |
| **Status** | **FIXED** — token blob writes now use synchronous persistence, and `KeystoreDeviceTokenProvider.storeTokens()` commits the device-token and refresh-token blobs together through a single atomic `storeTokenBlobs(...).commit()` path. |
| **Category** | Insecure Token Storage |
...
| **Module** | Security |
| **Severity** | Low |
| **Status** | **FIXED** — `KeystoreManager.getOrCreateKey()` now inspects `KeyInfo.isInsideSecureHardware()` once per alias, logs an explicit `INFO` confirmation for hardware-backed keys and a `WARN` when an alias is software-backed, and exposes `isHardwareBacked(alias)` for runtime checks. Added an instrumented smoke test that exercises the hardware-backing inspection path on a real device/emulator. |
| **Category** | Insecure Token Storage |
...
| **Module** | UI / Security |
| **Severity** | Medium |
| **Status** | **FIXED** — `ProvisioningActivity` and `SettingsActivity` now set `WindowManager.LayoutParams.FLAG_SECURE` at the start of `onCreate()` before rendering sensitive UI, preventing screenshots, recent-app thumbnails, and ordinary screen recording of provisioning tokens and FCC access codes. |
...
| **Severity** | Medium |
| **Status** | **FIXED** — Added release R8 rules to strip `android.util.Log.v/d/i` calls, introduced a debug-only `PlatformLogBridge`, and changed `AppLogger` / `StructuredFileLogger` to stop mirroring application logs into logcat in release builds. Release diagnostics remain in the structured JSONL files instead of ADB logcat. |
| **Category** | Sensitive Data Logged |
...
| **Severity** | Medium |
| **Status** | **FIXED** — Room now opens the buffer database through SQLCipher (`net.zetetic:android-database-sqlcipher:4.5.4`) via `BufferDatabaseFactory` and `BufferDatabaseEncryption`. A random SQLCipher passphrase is generated once, protected with the Android Keystore under `KeystoreManager.ALIAS_BUFFER_DB_PASSPHRASE`, persisted as encrypted bytes in app-private no-backup storage, and existing plaintext `fcc_buffer.db` files are migrated in place to an encrypted copy on first startup. Added an instrumented smoke test that verifies the resulting database file does not expose the plaintext SQLite header. |
...
| **Severity** | Medium |
| **Status** | **FIXED** — The global uncaught-exception handler and DI `CoroutineExceptionHandler` no longer interpolate raw `throwable.message` into log text. They now log fixed summaries and rely on `StructuredFileLogger`’s existing sanitization for persisted exception metadata, while the new debug-only `PlatformLogBridge` prevents release logcat from receiving raw exception payloads. |
| **Category** | Sensitive Data Logged |
...
| **Module** | Diagnostics & Monitoring |
| **Severity** | Low |
| **Status** | **FIXED** — `DiagnosticsActivity` now cleans up `edge-agent-logs*.zip` archives in `onCreate()`, again in `onResume()` after returning from the share flow, and before creating a new archive. The share action now uses an `ActivityResultLauncher`, so stale diagnostic ZIPs do not accumulate in `cacheDir`. |
| **Category** | Insecure File Handling |
