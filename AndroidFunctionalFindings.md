...
| **Status** | **Fixed** — (1) `DecommissionedActivity.startReProvisioning()` now calls `localOverrideManager.clearAllOverrides()` after `encryptedPrefs.clearAll()`, with `LocalOverrideManager` injected via Koin. (2) `RegistrationHandler.completeRegistration()` (which replaced direct ProvisioningViewModel credential handling per AT-014) now also calls `localOverrideManager.clearAllOverrides()` after clearing credentials, with `LocalOverrideManager` added as a constructor dependency. (3) `DiagnosticsViewModel` now collects active local overrides (FCC host, port, JPL port, credential, WS port) and surfaces them in `DiagnosticsSnapshot.activeOverrides`. `DiagnosticsActivity` renders a "Local Overrides" row in the Configuration section — yellow when overrides are active (listing which fields), green "None" when clean. AppModule DI updated for both `RegistrationHandler` and `DiagnosticsViewModel`. |
...
| **Status** | **Fixed** — `EncryptedPrefsManager.saveRegistration()` now returns `Boolean` (the result of `commit()`), and logs an error when `commit()` returns false. `RegistrationHandler.completeRegistration()` checks the return value and throws `IllegalStateException("Failed to persist registration data — disk may be full")` on failure, surfacing the error to the UI before the foreground service starts. |
...
| **Status** | **Fixed** — Added `internetInitialProbe` and `fccInitialProbe` flags to `ConnectivityManager`. On the very first probe for each channel, a single success immediately transitions to UP (bypassing `recoveryThreshold`). The flag is set to `false` after the first probe result (success or failure), so subsequent DOWN→UP recovery still requires the full `recoveryThreshold` (2) consecutive successes for anti-oscillation. Two new tests added in `ConnectivityManagerTest`: one verifying immediate transition on first successful probe, and one verifying that recovery from DOWN still requires the threshold after the initial bypass is consumed. |
