...
class DeviceTokenProviderTest {
    ...
    @Nested
    @DisplayName("getLegalEntityId")
    inner class GetLegalEntityId {

        @Test
        fun `returns value from encrypted prefs`() {
        ...
        @Test
        fun `returns null when not registered`() {
    ...
    inner class RefreshAccessToken {
        ...
        @Test
        fun `returns false when decommissioned`() = runTest {
            every { encryptedPrefs.isDecommissioned } returns true
            assertFalse(provider.refreshAccessToken())
        }

        @Test
        fun `returns false when no refresh token available`() = runTest {
            every { encryptedPrefs.getRefreshTokenBlob() } returns null
            assertFalse(provider.refreshAccessToken())
        }

        @Test
        fun `refreshes and stores new tokens on success`() = runTest {
        ...
        @Test
        fun `returns false on unauthorized (expired refresh token)`() = runTest {
            val refreshBlob = android.util.Base64.encodeToString(
                byteArrayOf(10, 20, 30),
                android.util.Base64.NO_WRAP,
            )
        ...
        @Test
        fun `marks decommissioned on 403 DEVICE_DECOMMISSIONED`() = runTest {
            val refreshBlob = android.util.Base64.encodeToString(
                byteArrayOf(10, 20, 30),
                android.util.Base64.NO_WRAP,
            )
        ...
        @Test
        fun `returns false on transport error`() = runTest {
            val refreshBlob = android.util.Base64.encodeToString(
                byteArrayOf(10, 20, 30),
                android.util.Base64.NO_WRAP,
            )
    ...
    @Nested
    @DisplayName("isDecommissioned / markDecommissioned")
    inner class Decommission {

        @Test
        fun `isDecommissioned delegates to encrypted prefs`() {
            every { encryptedPrefs.isDecommissioned } returns false
        ...
        @Test
        fun `markDecommissioned sets flag in encrypted prefs`() {
    ...
    inner class StoreTokens {

        @Test
        fun `encrypts and persists both tokens`() {
